...
import numpy as np
from busquedas_02 import argmax
...
class Juego():
    ...
    def resultado(self, estado, jugada_ejecutada_jugador_actual):
    ...
    def mover_a(self, estado):
        """Devuelve el jugado cuyo jugada_ejecutada_jugador_actual se encuentra en este estado."""
    ...
    def obtener_accion(self, jugador, estado, turno=1, verbose=0, **kwargs):
        ...
        """
        jugada_ejecutada_jugador_actual = jugador.obtener_accion(estado, turno=1, verbose=0, **kwargs)
        return jugada_ejecutada_jugador_actual

    def jugar_juego(self, *jugadores, lanzar_moneda=True, verbose=2, **kwargs):
        """Juega una n-persona, jugada_ejecutada_jugador_actual-alternado el juego."""
        if len(jugadores) == 0:
        ...
        for jugador in jugadores:
            jugada_ejecutada_jugador_actual = jugador.establecer_juego(self) ## ejecutar inicializacion, aqui se reinicia
        turno = 1
        while True:
            for jugador in jugadores:
                if verbose >= 2:
                    print("%s esta pensando..." % jugador.nombre)
                jugada_ejecutada_jugador_actual = self.obtener_accion(jugador, estado, turno, verbose, **kwargs)
                estado = self.resultado(estado, jugada_ejecutada_jugador_actual)
                if verbose >= 2:
                    print("%s ejecuta la acción %s:" % (jugador.nombre, jugada_ejecutada_jugador_actual))
                    self.mostrar(estado)
...
class TicTacToe(Juego):
    ...
    def resultado(self, estado, jugada_ejecutada_jugador_actual):
        if jugada_ejecutada_jugador_actual not in estado.movimientos:
            return estado  # Movimientos ilegales no tienen efecto
        tablero = estado.tablero.copy()
        tablero[jugada_ejecutada_jugador_actual] = estado.mover_a
        movimientos = list(estado.movimientos)
        movimientos.remove(jugada_ejecutada_jugador_actual)
        return EstadoJuego(mover_a=('O' if estado.mover_a == 'X' else 'X'),
                         utilidad=self.calcular_utilidad(tablero, jugada_ejecutada_jugador_actual, estado.mover_a),
    ...
    def calcular_utilidad(self, tablero, jugada_ejecutada_jugador_actual, jugador):
        """Si 'X' gana con este movimiento, devuelve 1; si 'O' gana, devuelve -1; de lo contrario, devuelve 0."""
        if (self.k_en_fila(tablero, jugada_ejecutada_jugador_actual, jugador, (0, 1)) or
                self.k_en_fila(tablero, jugada_ejecutada_jugador_actual, jugador, (1, 0)) or
                self.k_en_fila(tablero, jugada_ejecutada_jugador_actual, jugador, (1, -1)) or
                self.k_en_fila(tablero, jugada_ejecutada_jugador_actual, jugador, (1, 1))):
            return +1 if jugador == 'X' else -1
        else:
            return 0

    def k_en_fila(self, tablero, jugada_ejecutada_jugador_actual, jugador, delta_x_y):
        """Devuelve verdadero si hay una línea gracias al movimiento en el tablero para el jugador.."""
        (delta_x, delta_y) = delta_x_y
        x, y = jugada_ejecutada_jugador_actual
        ...
        while tablero.get((x, y)) == jugador:
            n += 1
            x, y = x - delta_x, y - delta_y
        n -= 1  # Porque contamos jugada_ejecutada_jugador_actual dos veces
        return n >= self.k
...
# ______________________________________________________________________________
# Clases de jugadores para los juegos
...
class JugadorHumano(Jugador):
    ...
    def obtener_accion(self, estado, turno=1, verbose=0):
        ...
        try:
            jugada_ejecutada_jugador_actual = eval(move_string)
        except NameError:
            jugada_ejecutada_jugador_actual = move_string
        return jugada_ejecutada_jugador_actual
